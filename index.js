...
var PointerTrap = require('./utils/pointer-trap-relative');
...
function FPSCameraController(camera, element, options) {
  ...
  var _movementSpeed = defined(options.movementSpeed, 0.1);
  var _movementRunSpeedScale = defined(options.movementRunSpeedScale, 2.5);
  var _rotationSpeed = defined(options.rotationSpeed, 0.005);
  var _viewlock = defined(options.viewlock, true);
  var _minFov = defined(options.minFov, 10);
  var _maxFov = defined(options.maxFov, 100);
  var _lockStatusCallback = defined(options.lockStatusCallback, () => {});
  var _zoomSpeed = defined(options.zoomSpeed, 0.001);
  var _yUp = defined(options.yUp, true);
  var _rotateActive = defined(options.rotateActive, false);
  var _rotateActiveOnlyInPointerLock = defined(
  ...
  if (_viewlock) {
    ...
    _pointerTrap.onAttainSignal.add(function() {
      if (_rotateActiveOnlyInPointerLock) {
        _rotateActive = true;
        _lockStatusCallback(true);
      }
    });
    _pointerTrap.onReleaseSignal.add(function() {
      if (_rotateActiveOnlyInPointerLock) {
        _rotateActive = false;
        _lockStatusCallback(false);
      }
    });
